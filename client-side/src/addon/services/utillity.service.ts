...
export class UtillityService {
    ...
    getRemoteLoaderOptions(tab: any, remoteEntry: string = '', type: 'script' | 'module' | 'manifest' = 'module'): PepRemoteLoaderOptions {
      const exposedModule = tab.elementsModule?.length > 0 ? `./${tab.elementsModule}` : tab.exposedModule;
      const res = {
          type: type,
          remoteEntry: remoteEntry.length > 0 ? remoteEntry : tab.remoteEntry,
